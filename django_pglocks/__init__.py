...
def advisory_lock(lock_id, shared=False, wait=True, using=None):
    ...
    if isinstance(lock_id, (list, tuple,)):
        ...
        if not isinstance(lock_id[0], integer_types) or not isinstance(lock_id[1], integer_types):
            raise ValueError("Both members of a tuple/list lock ID must be integers")

        tuple_format = True
    elif isinstance(lock_id, string_type):
        ...
        if pos & 2**31:
            lock_id -= 2**31
    elif not isinstance(lock_id, integer_types):
